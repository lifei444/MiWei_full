...
#pragma mark - - - 扫描外部View的创建
    ...
    CGFloat right_ViewX = CGRectGetMaxX(self.scanContentView.frame);
...
#pragma mark - - - 扫描边角imageView的创建
    ...
    UIImage *left_image = [UIImage imageNamed:@"qrcode_scan_top_left"];
    UIImageView *left_imageView = [[UIImageView alloc] init];
    CGFloat left_imageViewX = CGRectGetMinX(self.scanContentView.frame) - left_image.size.width * 0.5 + margin;
    ...
    // 右上侧的image
    UIImage *right_image = [UIImage imageNamed:@"qrcode_scan_top_right"];
    UIImageView *right_imageView = [[UIImageView alloc] init];
    CGFloat right_imageViewX = CGRectGetMaxX(self.scanContentView.frame) - right_image.size.width * 0.5 - margin;
    ...
    // 左下侧的image
    UIImage *left_image_down = [UIImage imageNamed:@"qrcode_scan_bottom_left"];
    UIImageView *left_imageView_down = [[UIImageView alloc] init];
    CGFloat left_imageView_downX = left_imageView.frame.origin.x;
    CGFloat left_imageView_downY =
    CGRectGetMaxY(self.scanContentView.frame) - left_image_down.size.width * 0.5 - margin;
    ...
    // 右下侧的image
    UIImage *right_image_down = [UIImage imageNamed:@"qrcode_scan_bottom_right"];
...
- (void)animation_line_action {
    ...
    if (flag) {
        ...
        if (_animation_line.frame.origin.y >= scanContent_Y) {
            CGFloat scanContent_MaxY = scanContent_Y + self.frame.size.width - 2 * scanContent_X - 10;
